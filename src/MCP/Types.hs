...
instance ToJSON TextContent where
  toJSON (TextContent _ txt anns) = object $ 
    [ "type" .= ("text" :: Text)
    , "text" .= txt
...
instance ToJSON ImageContent where
  toJSON (ImageContent _ dat mime anns) = object $
    [ "type" .= ("image" :: Text)
    , "data" .= dat
    , "mimeType" .= mime
    ] ++ maybe [] (\a -> ["annotations" .= a]) anns
...
instance ToJSON AudioContent where
  toJSON (AudioContent _ dat mime anns) = object $
    [ "type" .= ("audio" :: Text)
    , "data" .= dat
    , "mimeType" .= mime
    ] ++ maybe [] (\a -> ["annotations" .= a]) anns
...
instance ToJSON EmbeddedResource where
  toJSON (EmbeddedResource _ res anns) = object $
    [ "type" .= ("resource" :: Text)
    , "resource" .= res
    ] ++ maybe [] (\a -> ["annotations" .= a]) anns
...
instance ToJSON ResourceReference where
  toJSON (ResourceReference _ u) = object
...
instance ToJSON InputSchema where
  toJSON (InputSchema _ props req) = object $ 
    [ "type" .= ("object" :: Text)
...
instance ToJSON PromptReference where
  toJSON (PromptReference _ n) = object
