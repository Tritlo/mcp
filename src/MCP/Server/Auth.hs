...
import Network.HTTP.Simple (httpJSON, parseRequest, getResponseBody, setRequestMethod, setRequestBodyJSON, addRequestHeader)
...
instance FromJSON OAuthMetadata where
    parseJSON = Aeson.withObject "OAuthMetadata" $ \v -> OAuthMetadata
        <$> v Aeson..: "issuer"
...
instance ToJSON OAuthMetadata where
    toJSON OAuthMetadata{..} = Aeson.object $
        [ "issuer" Aeson..= issuer
...
instance FromJSON TokenInfo where
    parseJSON = Aeson.withObject "TokenInfo" $ \v -> TokenInfo
        <$> v Aeson..: "active"
        <*> v Aeson..:? "scope"
        <*> v Aeson..:? "client_id"
        <*> v Aeson..:? "username"
...
introspectToken endpoint token = liftIO $ do
    ...
    let requestWithBody = setRequestMethod "POST"
